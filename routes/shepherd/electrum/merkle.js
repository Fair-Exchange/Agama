    ...
    return new Promise((resolve, reject) => {
      if (_serverList !== 'none') {
        ...
        shepherd.verifyMerkle(
          ...
          height,
          _filteredServerList,
          shepherd.electrumCoins[coin].server.ip + ':' + shepherd.electrumCoins[coin].server.port + ':' + shepherd.electrumCoins[coin.toLowerCase() === 'safe' || coin === 'safecoin' ? 'safe' : coin].server.proto,
          coin
        )
        .then((proof) => {
          resolve(proof);
