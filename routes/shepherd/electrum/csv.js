    ...
    if (shepherd.checkToken(req.query.token)) {
      ...
      .then((txhistory) => {
        ...
        if (txhistory.msg === 'success' &&
          ...
          const err = fs.writeFileSync(`${shepherd.safewalletDir}/shepherd/csv/${_coin.toUpperCase()}-${req.query.address}-${_time}.csv`, _csv.join('\r\n'), 'utf8');
          ...
          if (err) {
            ...
            res.end(JSON.stringify({
              msg: 'success',
              result: `${shepherd.safewalletDir}/shepherd/csv/${_coin.toUpperCase()}-${req.query.address}-${_time}.csv`,
            }));
          }
        } else {
      ...
      request(options, (error, response, body) => {
        try {
          ...
          if (!txhistory.error &&
            ...
            const err = fs.writeFileSync(`${shepherd.safewalletDir}/shepherd/csv/${_coin.toUpperCase()}-native-all-${_time}.csv`, _csv.join('\r\n'), 'utf8');
            ...
            if (err) {
              ...
              res.end(JSON.stringify({
                msg: 'success',
                result: `${shepherd.safewalletDir}/shepherd/csv/${_coin.toUpperCase()}-native-all-${_time}.csv`,
              }));
            }
          } else {
            res.end(JSON.stringify({
