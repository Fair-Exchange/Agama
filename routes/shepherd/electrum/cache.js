...
module.exports = (shepherd) => {
  shepherd.loadLocalSPVCache = () => {
    if (fs.existsSync(`${shepherd.safewalletDir}/spv-cache.json`)) {
      let localCache = fs.readFileSync(`${shepherd.safewalletDir}/spv-cache.json`, 'utf8');
  ...
  shepherd.saveLocalSPVCache = () => {
    let spvCacheFileName = `${shepherd.safewalletDir}/spv-cache.json`;
      ...
      if (!err) {
        ...
        const FsWrite = () => {
          return new Promise((resolve, reject) => {
            ...
            setTimeout(() => {
              shepherd.log(result);
              shepherd.log(`spv-cache.json file is created successfully at: ${shepherd.safewalletDir}`);
              resolve(result);
            }, 2000);
          });
        }
